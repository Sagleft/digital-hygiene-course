...
func getSourceAnswer3() tgfun.FunnelEvent {
	return tgfun.FunnelEvent{
		Message: tgfun.EventMessage{
			...
			Знакомые и друзья могут ошибиться, они могут не разбираться в теме статьи, не заметить в ней странности, которые говорят о том, что эта информация недостоверна.`,
			Buttons: []tgfun.MessageButton{
				getTip4Button(),
			},
			ButtonsIsColumns: true,
...
func getSourceAnswer4() tgfun.FunnelEvent {
	return tgfun.FunnelEvent{
		Message: tgfun.EventMessage{
			...
			В интернете нас постоянно атакуют информацией, в которую вплетен чей-то интерес. Интернет полон настоящих медиа-вирусов, призванных уверить тебя в чем-либо, чтобы использовать твоё внимание.
