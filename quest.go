...
func getEventAboutAI2() tgfun.FunnelEvent {
	return tgfun.FunnelEvent{
		Message: tgfun.EventMessage{
			Text: `😎 В *Utopia* ты можешь получить бесплатный доступ к ИИ-ассистенту, который подскажет как решить любую проблему, найдет для тебя информацию и даже поможет сгенерировать какую-либо идею.
